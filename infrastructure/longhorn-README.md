...
| `longhorn-backup` | Velero and backup data | Node 203 capacity disks; `/dev/sdi` is excluded until wiped intentionally |
...
Run `scripts/longhorn-validate-disks.sh` before applying patches. Any partition warning must be treated as a blocker until the disk contents are verified. Node 203 `/dev/sdi` currently has existing partitions and is not mounted for Longhorn.
